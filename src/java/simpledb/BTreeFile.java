...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if(entry==null) throw new DbException("Entry is null.");
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        //return null;
		BTreeLeafPage rightP = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		int mid = (page.getNumTuples() + 1) / 2;
		Iterator<Tuple> iter = page.reverseIterator();
		Tuple tuple = null;
		for (int i = 0; i < mid && iter.hasNext(); i++) {
			tuple = iter.next();
			page.deleteTuple(tuple);
			rightP.insertTuple(tuple);
		}
		Field midField = tuple.getField(this.keyField());
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midField);
		parentPage.insertEntry(new BTreeEntry(midField, page.getId(), rightP.getId()));
		rightP.setParentId(parentPage.getId());
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage rSibling = (BTreeLeafPage)(this.getPage(tid, dirtypages,
					page.getRightSiblingId(), Permissions.READ_WRITE));
			rSibling.setLeftSiblingId(rightP.getId());
			rightP.setRightSiblingId(rSibling.getId());
		}
		page.setRightSiblingId(rightP.getId());
		rightP.setLeftSiblingId(page.getId());


		if (field.compare(Op.LESS_THAN_OR_EQ, midField)) return page;
		return rightP;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage rightP = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		Iterator<BTreeEntry> iter = page.reverseIterator();
		int mid = (page.getMaxEntries() -1) / 2;
		BTreeEntry entry;
		for (int i = 0; i < mid; i++) {
			entry = iter.next();
			page.deleteKeyAndRightChild(entry);
			rightP.insertEntry(entry);
		}
		entry= iter.next();
		Field entryKey=entry.getKey();
		BTreeInternalPage emptyP = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);

		page.deleteKeyAndRightChild(entry);

		entry.setLeftChild(page.getId());
		entry.setRightChild(rightP.getId());
		emptyP.insertEntry(entry);
		rightP.setParentId(emptyP.getId());

		updateParentPointers(tid,dirtypages, rightP);
		if(field.compare(Op.LESS_THAN_OR_EQ, entryKey)) return page;
		return rightP;
	}
